...
import 'alta_cipriana.dart';

class catalogo_cipriana extends StatefulWidget {

  cajas_modelo product;

  final int folio;
  catalogo_cipriana(this.folio, this.product);


  @override
  catalogo_ciprianaState createState() => catalogo_ciprianaState();
}

class catalogo_ciprianaState extends State<catalogo_cipriana> {
  ...
  Widget lista (){
    ...
    return Expanded(
      child: StreamBuilder(
        ...
        //Y ASI  CON TODOS
          stream: reflistaproduccion.where('correoNegocio', isEqualTo: correo).where('categoria', isEqualTo: "Cipriana").orderBy('nombreProducto', descending: false).snapshots(),
      ...
      floatingActionButton: SpeedDial( //Boton flotante animado
        ...
        children: [
          ...
          SpeedDialChild(
              ...
              backgroundColor: Colors.brown,
              label: 'Agregar Cipriana',
              onTap: () async {

                await Navigator.push(context, MaterialPageRoute(builder: (context) => alta_cipriana(cajas_modelo("", widget.product.nombreProducto,"fecha",0,2,3, 0,5,"","Hombres","foto","descripcion", "newid", 0))),);
              }
          ),
          //  SpeedDialChild(
